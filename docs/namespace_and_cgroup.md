...
6/Pid Namespace
    ...
    （2022.09.06）需要更改的系统调用，主要考虑到已经实现了的需要更改的，有sys_getpid，sys_gettid，sys_getppid，sys_set_tid_address。需要注意这个getppid，就是得到父进程的pid，如果当前进程是某个pidns的根进程，那么不应该看见其父进程的pid，换句话说，在当前pidns下，在他看来，他就是根进程。另外，在创建新的进程和新的线程的时候，需要插入数据结构。

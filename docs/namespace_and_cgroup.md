...
11/如何调用系统调用进行测试的记录
    ...
    其次是，需要装个交叉编译器，这个坑我是不想踩第二遍了，也不希望有人踩第二遍。简单来说，他官方的那个需要装一大堆子模块，本来网速就不好，笼笼统统需要好几个g，能下载完就有鬼了，如果有科学上网，强烈推荐挂上。如果没有科学上网，我找到了个虽然可能年代久远，但是应该是能用的，https://zhuanlan.zhihu.com/p/259305354，这老哥把需要的文件塞里面了，总共几百个m。

    需要注意的是，因为他用的是一个zcore编译进了系统之后的so文件，相当于使用动态链接（废话，内核都改了，那主页上的什么cargo libc-test --arch riscv64命令，因为非qemu下没有那个so文件，根本玩不转。），所以make的时候用make linux的那个命令去弄linux-gnu-gcc即可。至于那个pk和spike子模块，但凡你用的是qemu都完全不用管。

    装完配环境变量去。riscv64-unknown-linux-gnu -v能过就是编译过了。
